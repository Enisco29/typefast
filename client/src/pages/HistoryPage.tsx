      ...
      <div className="w-[90%] max-w-[1100px]">
        ...
        {!history.length ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {history.map((h) => (
              <div key={h.id} className="p-5 rounded-lg border border-gray-200 bg-white shadow-sm hover:shadow-md transition-shadow">
                <div className="flex items-center justify-between mb-3">
                  <p className="text-gray-500 text-sm">{formatDate(h.dateISO)}</p>
                  <button onClick={() => deleteEntry(h.id)} className="text-red-600 text-sm hover:underline">Delete</button>
                </div>
